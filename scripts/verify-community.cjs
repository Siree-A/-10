...
(async()=>{
 ...
 await page.addInitScript(()=>localStorage.setItem('research10.tour.v1','done'));await page.goto(base+'/game.html');await page.waitForFunction(()=>document.querySelector('#member-status').textContent.includes('80'));
 ...
 await cp.addInitScript(()=>{try{localStorage.setItem('research10.tour.v1','done');}catch{}});await cp.goto(base+'/game.html');await cp.waitForFunction(()=>document.querySelector('#ranking-status').textContent.includes('ออนไลน์'));assert.equal(signup,0);
 ...
 await cp.waitForFunction(()=>document.querySelector('#cloud-message').textContent.includes('รอส่ง'));assert.equal(signup,1);assert.equal(records.length,1);assert.equal(records[0].p_code,'A12');assert.equal(records[0].points,undefined);

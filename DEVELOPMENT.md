...
`nBefore publishing CSS or JavaScript changes, run python scripts/version-assets.py. It updates local asset URLs with content hashes so cached older styles cannot mismatch new HTML. Run node scripts/verify-leadership.cjs to assert that both chair cards align side by side at seven viewport widths. Set SITE_URL to the public site for post-deployment checks.

...
export function createClub(hooks){
  let period='daily',track='all',onlineRows=null,onlineMessage='',busy=false,activePlayer=null,breakTimer,breakEnd=0;
  function ranking(){
    ...
    $('ranking-status').textContent=onlineRows?'อันดับรวมออนไลน์ · อัปเดตล่าสุดเมื่อกดรีเฟรช':onlineMessage||'อันดับเฉพาะเครื่องนี้ · ยังไม่เชื่อมฐานข้อมูลกลาง';
  ...
  async function refresh(){
    ...
    finally{$('refresh-ranking').disabled=false;ranking();}
  ...
  async function flush(p){
    if(busy||!configured||!p.cloudJoin)return;busy=true;
    try{
      ...
      await refresh();
    }catch{if(p===hooks.getPlayer())$('cloud-message').textContent=`ยังส่งไม่สำเร็จ ${p.outbox?.length||0} รายการ · กดรีเฟรชเพื่อลองใหม่ภายในวันนี้`;}
    finally{busy=false;}
  }
  ...
  $('survey-form').addEventListener('submit',async e=>{
    ...
    if($('share-survey').checked&&configured){const submit=e.target.querySelector('[type=submit]');submit.disabled=true;try{await rpc('quest_reflect',{p_relax:record.relax,p_learn:record.learn,p_satisfaction:record.satisfaction,p_topic:record.topic});$('survey-status').textContent='✓ ส่งความคิดเห็นแล้ว ไม่มีชื่อหรือรหัสสมาชิกในแบบประเมิน';}catch{$('survey-status').textContent=stored?'เก็บผลไว้ในเครื่องแล้ว แต่ยังส่งออนไลน์ไม่ได้ ลองบันทึกอีกครั้ง':'ยังบันทึกและส่งไม่ได้ กรุณาส่งออกผลก่อนปิดหน้า';}finally{submit.disabled=false;}}
  });
  ...
  function update(p){
    if(activePlayer!==p){activePlayer=p;surveyValues=null;$('survey-form').reset();$('join-ranking').checked=!!p?.cloudJoin;surveySummary();if(p?.cloudJoin)flush(p);}
